...
main () {
  ...
  Usage: check-res <url> <optional - iteration>
...
if [[ -z "$iteration" ]]; then
    iteration=10;
    echo "WARN: iteration to default $iteration."
...
print_header () {
  echo "code,time_total,time_connect,time_appconnect,time_starttransfer,size_downloaded,speed_download,http_version"
}
...
make_request () {
  ...
  curl \
    --write-out "%{http_code},%{time_total},%{time_connect},%{time_appconnect},%{time_starttransfer},%{size_download},%{speed_download},%{http_version}\n" \
    --silent \
    --output /dev/null \
    "$url" 
}
